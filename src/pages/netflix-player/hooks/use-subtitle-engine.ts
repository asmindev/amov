...
interface UseSubtitleEngineOpts {
...
/**
 * Subtitle engine for the player rewrite.
 *
 * Ports use-subtitles.ts and use-auto-select-subtitle.ts to the reducer:
 * - auto-selects the default (Indonesian) subtitle when none is chosen
 * - fetches the selected subtitle, shifts timestamps by subOffset, wraps it
 *   as a WebVTT blob URL and parses cues (behavior preserved from the old hook)
 * - dispatches SET_VTT / SET_CUES / SET_SUB_ERROR / SET_SUB to the store
 *
 * `currentCues` in the store holds the cues active at `currentTime`, recomputed
 * on every timeupdate (mirrors the old useMemo on [parsedCues, currentTime]).
 */
export function useSubtitleEngine({
  ...
  // Auto-select a default subtitle (Indonesian) when none is chosen yet.
  useEffect(() => {
    if (subtitles.length > 0 && selectedSub === null) {
      ...
      if (defaultSub) {
        dispatchRef.current({ type: "SET_SUB", url: defaultSub.url })
      }
    }
  }, [subtitles, selectedSub])

  // Fetch, shift, blob-ify and parse the selected subtitle.
  useEffect(() => {
    let isCancelled = false

    async function fetchAndParse() {
      if (!selectedSub) {
        dispatchRef.current({ type: "SET_CUES", cues: [] })
        dispatchRef.current({ type: "SET_VTT", url: null })
      ...
      try {
        ...
        dispatchRef.current({ type: "SET_VTT", url })
      } catch (err) {
        console.error("Subtitle parse error", err)
        if (!isCancelled) {
          setParsedCues([])
          dispatchRef.current({ type: "SET_SUB_ERROR", error: true })
          dispatchRef.current({ type: "SET_VTT", url: null })
          dispatchRef.current({ type: "SET_CUES", cues: [] })
        }
      }
  ...
  // Dispatch the cues active at currentTime.
  useEffect(() => {
    ...
    dispatchRef.current({ type: "SET_CUES", cues: result })
